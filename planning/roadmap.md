...
|---|---|---|---|---|
| Q1 | Does the curated `ApprovalMode.auto_review` route every write-class Codex tool call to `approval_handler`, or only ones the app-server's own auto-reviewer escalates? | If escalation-only: force `ApprovalsReviewer.user` via `thread_start`'s raw `config={...}` passthrough instead of the curated enum. Exact config key unverified - confirm empirically against a real `codex app-server` first. | Phase 3 | OPEN |
| Q2 | What is the current config schema for pointing Codex at a remote MCP-over-HTTP server, given `codex mcp-server`/`codex-mcp-server` was removed? | Confirm against `https://learn.chatgpt.com/docs/mcp` (or equivalent) and the pinned runtime version before building the bridge. | Phase 3 | OPEN |
...
| Q4 | Exact wire shape omp expects for `local_api_key`-bearing custom providers (is `models.yml`'s `apiKey` a literal value, an env-var reference, or both)? | Check `omp://providers.md`'s custom-provider schema section in full before writing `OmpSession`'s config generation. | Phase 4 | OPEN |
