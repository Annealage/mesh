...
_IMAGE_NAME_RE = re.compile(
    r"^[A-Za-z0-9][A-Za-z0-9._-]{0,95}\.(png|jpg|jpeg|webp)$", re.IGNORECASE
)
...
# What a file this package creates under ``review/`` may be called. The name
# is generated by this process, never a client, so the pattern exists only to
# keep every name this package writes recognisably its own rather than to
# contain arbitrary input the way ``_IMAGE_NAME_RE`` does.
_REVIEW_NAME_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}\.(md|jsonl)$")

# Mode for a transcript this package creates under review/. Matches
# _IMAGE_FILE_MODE: both are meant to be committed and looked at, not kept
# private to the process that wrote them.
_REVIEW_FILE_MODE = 0o644
...
def open_fixed_file_for_append(serve_dir, name, mode):
    ...
    * the same inode still present at the name afterwards, which closes the
    ...
    if target is None:
        return None
    try:
        fd = os.open(target, os.O_WRONLY | os.O_CREAT | os.O_APPEND | OPEN_GUARD_FLAGS, mode)
    except OSError:
        return None
    try:
        st = os.fstat(fd)
...
def create_review_file(serve_dir, name):
    """Create ``serve_dir``/review/``name`` for writing and return its descriptor.

    Returns None when the name does not match ``_REVIEW_NAME_RE`` or the
    ``review`` entry is not something safe to write into, and raises
    ``FileExistsError`` when that exact name is already taken. Callers pass a
    generated name, never one a client chose, but a generated name is not by
    itself a reason to skip containment: ``review/`` sits in the same served
    project directory as ``images/``, which this process does not otherwise
    trust, so the checks below hold regardless of where the name came from.

    ``review/`` is created if absent, and refused if it is a symlink, exactly
    as ``create_image_file`` treats ``images/``. What differs is that the
    open below carries ``O_EXCL`` and ``O_NOFOLLOW`` on the file itself, which
    means the file this call creates cannot have pre-existed or been a link,
    but says nothing about ``review/`` itself, an intermediate path
    component O_NOFOLLOW never inspects: replacing that directory with a
    symlink between the check above and the open below still redirects the
    write through the new link. Recording ``review/``'s identity before the
    open and requiring it unchanged afterwards closes that window: a swap in
    between is caught, and the file the open created is removed rather than
    left behind at whatever the link pointed to.
    """
    serve_dir = resolve_serve_dir(serve_dir)
    if not _REVIEW_NAME_RE.match(name or ""):
        return None
    review_dir = serve_dir / REVIEW_DIRNAME
    ...
    flags = os.O_WRONLY | os.O_CREAT | os.O_EXCL | getattr(os, "O_NOFOLLOW", 0)
    fd = os.open(target, flags, _REVIEW_FILE_MODE)
    try:
