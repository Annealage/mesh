...
**Git config includes are followed, because git follows them.** The guard above gates a config that names a command, and `include.path` counts as one, so a config whose only content is an include is gated. What that alone left open was the file on the other end: its content was not part of the digest, so editing it changed what git runs while what had been accepted stayed equal. The digest now covers the whole include chain, each member named by its resolved path as well as its content, so re-pointing an include at a different file with identical bytes is also a change. Bounded at git's own limit of ten, and cycle-safe, since a config that includes itself is a shape a hostile archive can ship precisely because a naive follower loops on it.

`includeIf` conditions are deliberately not evaluated. Reproducing git's predicates (`gitdir`, `onbranch`, `hasconfig`) against the state of the moment would mean sometimes being wrong in the permissive direction, which means not watching a file git reads; watching one git ignores costs only a digest that changes more often. A missing include target is guarded too, and stays in the digest as absent, because the file appearing later is exactly how a config that looked harmless at startup starts running something.

One thing was measured rather than assumed here as well, and it corrected the tests rather than the code: a relative include resolves against the *including file's* directory, so `../elsewhere` written in `.git/config` lands inside the repository rather than beside it. Checked against real git, which reported the inside file's value.
...
**`/submit` was unauthenticated, and shipped that way in 2.0.0.** Section 3.10 says the per-run token is required on `/ws`, `/upload`, `/settings` and `/submit` in agent mode. The first three had it; `register_routes` was never given the token or the origin list, so the fourth had neither check in any mode. Found by an external review of the published tree and reproduced before being believed: a POST with no token, `Origin: https://evil.example` and a plain content type returned 200 and wrote the payload into `mesh-comments.json`, while `/upload` next door returned 403 for the same caller. That file is the documented human-to-agent channel and `SKILL.md` tells an agent to re-read it, so any page the human had open in another tab could put text into the instruction channel of an agent holding a shell, and on a tailnet bind every peer could.

The fix splits the two checks rather than applying both everywhere, because they answer different questions. The **`Origin` check is unconditional**: a POST carrying a plain body is a CORS-simple request, so no preflight stands in front of it, and the browser refusing to let the page read the reply protects nothing when the write itself is the payload. The **token is required in agent mode only**, which is what section 3.10 says, because the published skill flow tells the human to open the bare `http://localhost:8765/` and press Submit; requiring a token there would break the documented flow for a run with no agent attached to it.

Fixing the route broke the button, which is the part worth recording. `js/pins.js` posted `/submit` with no token, and every existing browser test of Submit runs against a server built with no session id, which is viewer mode, where the token is not required: the whole suite stayed green with the human's own Submit button broken for every real agent-mode run. The browser now sends the token, and a browser test runs against an agent-mode server specifically, confirmed to fail without it.
...
- Sandbox-level read restriction, which is what would close the remaining half of the credential refusal. The `Bash` half of that control is textual (see the record in section 4), so a path built from a glob or an encoding gets through; closing it means intercepting where paths are already resolved, which on Linux means the sandbox's own filesystem policy rather than a hook.
