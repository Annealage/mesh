...
- **A served directory's `.git/config` is not gated, and it can execute shell commands.** The workspace-trust gate (facts 21 to 24) covers `.claude/` and `.mcp.json`, which are the files the agent's CLI reads. Git has its own equivalent: a repository's own config can name commands git runs, through `core.fsmonitor`, `core.pager`, aliases and `include.path`, so running any git command inside an untrusted directory is arbitrary code execution on the same footing. M8 scaffolds after the trust gate for that reason, which orders the two correctly but does not close the hole, since the gate never looks at `.git/`. Closing it properly means either extending the digest to `.git/config` and its includes, or running scaffold-time git with the config sources it honours restricted. Left undone deliberately rather than half-done: the ordering is the cheap part and the digest change wants its own tests and its own decision about what a re-prompt costs someone who edits their own git config.
