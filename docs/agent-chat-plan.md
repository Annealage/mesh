...
**The read/write classification is one list, and building refuses to proceed without it.** `tools/registry.py` holds both tuples, `session/sdk.py` re-exports the read-class half rather than restating it, and `MeshTools` raises at construction if a built tool is unclassified, a classified name is not built, or a name appears in both. The alternative was a tool added to a handler module quietly defaulting into one posture or the other, and both defaults are wrong: read-class silently removes the human's approval card, and write-class is a name the allow list can never match.
...
- Flat tool list, read-class pre-allowed via `allowed_tools`, write-class always prompting. The classification lives in exactly one place (`tools/registry.py`) and a tool that is not in it does not build, because both silent defaults are wrong.
- **The pause switch is server-side state.** It gates write-class mesh tools only, it is set by an inbound `pause` frame rather than by a browser-local toggle, and the browser's control shows what the server reports rather than what was clicked. A control that latched locally would read "paused" while the tools it claims to gate were still running, which is what the first attempt at it actually did.
